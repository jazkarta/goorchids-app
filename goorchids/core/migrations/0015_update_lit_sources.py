...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for taxon in orm['core.TaxonCharacterValue'].objects.all():
            lit_source = taxon.lit_source
            if lit_source:
                new_cit, created = orm['core.SourceCitation'].objects.get_or_create(
                    citation_text=lit_source)
                taxon.literary_source = new_cit
                taxon.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for taxon in orm['core.TaxonCharacterValue'].objects.all():
            lit_source = taxon.literary_source
            if lit_source:
                taxon.lit_source = lit_source.citation_text
                taxon.literary_source = None
                taxon.save()
        orm['core.SourceCitation'].objects.all().delete()
